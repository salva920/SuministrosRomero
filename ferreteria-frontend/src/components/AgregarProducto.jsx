...
const AgregarProducto = ({ open, onClose, productoEditando, onProductoGuardado, agregarProductoAlEstado }) => {
  ...
  const handleAgregarProducto = async () => {
    try {
      // 1. Enviar la solicitud al backend para agregar el producto
      const response = await axios.post(`${API_URL}/productos`, nuevoProducto);

      // 2. Actualizar el estado en GestionInventario
      agregarProductoAlEstado(response.data); // Llamar a la función pasada como prop

      // 3. Mostrar mensaje de éxito
      toast.success('Producto agregado correctamente');

      // 4. Cerrar el modal de agregar producto
      onClose();
